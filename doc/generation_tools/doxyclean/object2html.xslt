	...
	<xsl:template match="/">
		...
		<body>
			<header id="projectHeader">
				<h1><a href="../index.html">VRToolKit Reference Library</a></h1>
			</header>
			...
			<footer id="breadcrumbs">
				<ul>
					<li><a href="../index.html">ARApi</a></li>
	...
	<xsl:template match="ref">
		<code>
			<xsl:choose>
				<xsl:when test="/object/name != child::node()[1]">
					<!--<a>
						<xsl:attribute name="href"><xsl:value-of select="current()"/>.html</xsl:attribute>
						<xsl:apply-templates/>
					</a>-->
					<strong>
						<xsl:apply-templates/>
